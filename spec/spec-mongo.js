...
describe("test", () => {
  ...
  describe("$and", () => {
    it("should match two positives", () => {
      y(
        { foo: { bar: "123" } },
        { $and: [{ foo: { $exists: true } }, { "foo.bar": { $eq: "123" } }] }
      );
      y(
        { foo: { bar: "123" } },
        { $and: [{ foo: { $exists: true } }, { "foo.bar": "123" }] }
      );
    });
    ...
    it("should not match one positive and one negative", () => {
      n(
        { foo: { bar: "123" } },
        { $and: [{ foo: { $exists: true } }, { "foo.bar": { $eq: "1234" } }] }
      );
      n(
        { foo: { bar: "123" } },
        { $and: [{ foo: { $exists: true } }, { "foo.bar": "1234" }] }
      );
    });
  ...
  describe("$eq", () => {
    ...
    it("should match ext json", () => {
      y(
        { updatedAt: { $date: 1413850114241 } },
        { "updatedAt.$date": { $eq: 1413850114241 } }
      );
      y(
        { updatedAt: { $date: 1413850114241 } },
        { "updatedAt.$date": 1413850114241 }
      ...
      n(
        { updatedAt: { $date: 1413850114241 } },
        { "updatedAt.$date": { $eq: 1413850114242 } }
      );
      n(
        { updatedAt: { $date: 1413850114241 } },
        { "updatedAt.$date": 1413850114242 }
      );
      n(
        { updatedAt: { $date: 1413850114241 } },
        { "updatedAt.$date": { $ne: 1413850114241 } }
      );
    });
  ...
  describe("$first", () => {
    it("should match first element of array", () => {
      y({ items: [1, 2, 3] }, { items: { $first: 1 } });
      y({ items: ["a", "b", "c"] }, { items: { $first: "a" } });
      n({ items: [1, 2, 3] }, { items: { $first: 2 } });
    });

    it("should not match for empty array", () => {
      n({ items: [] }, { items: { $first: 1 } });
    });

    it("should not match for non-array", () => {
      n({ items: "not an array" }, { items: { $first: 1 } });
    });
  });

  describe("$last", () => {
    it("should match last element of array", () => {
      y({ items: [1, 2, 3] }, { items: { $last: 3 } });
      y({ items: ["a", "b", "c"] }, { items: { $last: "c" } });
      n({ items: [1, 2, 3] }, { items: { $last: 2 } });
    });

    it("should not match for empty array", () => {
      n({ items: [] }, { items: { $last: 1 } });
    });

    it("should not match for non-array", () => {
      n({ items: "not an array" }, { items: { $last: 1 } });
    });
  });

  describe("$pop", () => {
    it("should match last element of array", () => {
      y({ items: [1, 2, 3] }, { items: { $pop: 3 } });
      y({ items: ["a", "b", "c"] }, { items: { $pop: "c" } });
      n({ items: [1, 2, 3] }, { items: { $pop: 2 } });
    });
    ...
    it("should not match for non-array", () => {
      n({ items: "not an array" }, { items: { $pop: 1 } });
    });
  });
});
